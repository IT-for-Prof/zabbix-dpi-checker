...
Зонды запускаются с нескольких **точек наблюдения** (далее — *vantage*, дословно
«точка обзора») в разных регионах и у разных провайдеров и классифицируют сбои
по DPI-характерным паттернам. Алерт поднимается только если **расхождение между
vantage'ями подтверждается DPI-сигнатурой**: часть точек видит блок, остальные —
OK, причём хотя бы одна из «не-OK» точек зафиксировала характерный признак
DPI (RST после ClientHello, заглушка-страница, DNS-подмена и т. п.). Это
позволяет отделить направленную блокировку от общего падения сервиса.

### Глоссарий ключевых терминов

- **Vantage** — отдельная Zabbix-точка наблюдения (server или proxy), пробивающая
  все таргеты независимо. В коде/шаблонах используется английский термин.
- **Phantom-host** — Zabbix-хост без агента и интерфейсов, нужен только чтобы
  держать триггеры и calculated items.
- **Discriminator** — флаг «у этого vantage'а сработал HIGH-confidence DPI-признак».
- **Гард / защитное условие** — дополнительная проверка в trigger expression
  (например, `vantages_total ≥ 3`), чтобы тиры severity не «наезжали» друг на
  друга при разных размерах mesh-а.
- **Debounce** — задержка срабатывания: триггер требует подтверждения N подряд
  замерами, чтобы единичный glitch не вызывал alert.
...
flowchart TB
    subgraph Server["Zabbix Server"]
        T1["Template:<br/>DPI Mesh Blocking Detection<br/>(vantage role — data layer)"]
        T2["Template:<br/>DPI Mesh Aggregator<br/>(consensus layer)"]
    end

    subgraph Vantages["DPI Vantages — где исполняются probe'ы"]
        V1[vantage A<br/>region EU]
        V2[vantage B<br/>region BY]
        V3[vantage C<br/>region RU, ISP-1]
        V4[vantage D<br/>region RU, ISP-2]
        V5[vantage E<br/>region RU, ISP-3]
    end

    subgraph Targets["DPI Targets — что мониторим"]
        TG1[example.com<br/>https:443]
        TG2[mail.example.com<br/>smtp:25, smtps:465]
        TG3[vpn.example.com<br/>wireguard:51820]
        TG4["..."]
    end

    Agg["dpi-mesh-aggregator<br/>(phantom host, 1 шт.)"]

    T1 -.linked.-> Vantages
    T2 -.linked.-> Agg
    Vantages -- "probe (cross-product)" --> Targets
    Vantages -- "dpi.verdict[T,K,P]" --> Agg
```

> Каждый vantage пробивает все таргеты независимо. Cross-vantage сравнение
> выполняется на aggregator-хосте через calculated items (`dpi.peers_ok`,
> `dpi.affected`, `dpi.vantages_total`, `dpi.discriminator_any`). По этим
> агрегатам разворачивается 4-уровневая severity-шкала
> (**Info / Warning / Average / High**), которая различает локальный glitch,
> directional-блок и глобальный outage. Триггеры дополнительно ограничены
> `last(dpi.discriminator_any) ≥ 1` — alert поднимается только при
> подтверждённой HIGH-confidence DPI-сигнатуре.
...
4. **Per-vantage Info-уровневый триггер `DPI vantage probe stale`** — диагностический,
   срабатывает если LLD/probe данного vantage'а молчит дольше `{$DPI.NODATA_WINDOW}`.
   Severity-триггеров на vantage-уровне **нет**.

**На aggregator-хосте** `dpi-mesh-aggregator` (см. ниже подробнее):

- Calculated items `dpi.peers_ok`, `dpi.affected`, `dpi.vantages_total`,
  `dpi.discriminator_any` агрегируют состояние всех vantages через
  `count(last_foreach(/*/dpi.verdict[…]), …)`.
- 4-tier severity ladder (Info → Warning → Average → High) + Info-sentinel
  «stale» срабатывают по числу `affected` после debounce-окна в 3
  последовательных измерения. Старшая ступень **High** покрывает в том
  числе полный outage (`peers_ok=0`) с подтверждённой DPI-сигнатурой —
  параллельно стандартным availability-шаблонам, но другим каналом.
...
1. **`DPI Mesh Blocking Detection`** (vantage role, привязывается к каждому
   probe-хосту) — data layer: только LLD + master/dependent items
   (`dpi.verdict`, `dpi.reason`, `dpi.latency_ms`, ...). **Severity-триггеров
   нет**, есть только per-vantage Info-уровневый stale-data триггер.
2. **`DPI Mesh Aggregator`** (привязывается к ОДНОМУ phantom-хосту
   `dpi-mesh-aggregator`) — consensus layer: пять calculated items per
   `(target, kind, port)` + 4-tier severity ladder + Info-sentinel
   «stale». `VANTAGE_UNAVAILABLE` исключается из `dpi.affected` и
   `dpi.vantages_total`, чтобы временно неработающий vantage не создавал
   ложный target-consensus.

Зачем aggregator-хост: если оставить severity-триггеры на vantage-template,
они материализуются на каждом vantage, и при глобальном outage будут
**срабатывать N раз** — оператор получает N одинаковых уведомлений.
Aggregator держит триггер в одном месте, поэтому один outage = один alert.
Calculated items на aggregator-е читают `/*/dpi.verdict[T,K,P]` (last_foreach
across all hosts) — данные остаются на vantages, агрегация
централизована.

Четыре calc items на aggregator-е:
- `dpi.peers_ok[T,K,P]` = `count(last_foreach(/*/dpi.verdict[T,K,P]),"eq","OK")`
- `dpi.affected[T,K,P]` = `count(last_foreach(...),"ne","OK")`
- `dpi.vantages_total[T,K,P]` = `count(last_foreach(...),"regexp",".")`
- `dpi.discriminator_any[T,K,P]` = `count(last_foreach(...),"regexp","^(DNS_LIE|TCP_RST_MID_STREAM|TLS_RESET_POST_HELLO|CERT_MISMATCH|HTTP_STUB)$")` —
  число vantage'ей, чей verdict — HIGH-confidence DPI-fingerprint
  (по `probe/lib/verdict.py:_DEFAULT_CONFIDENCE`).

> **Почему `affected` не вычитает `VANTAGE_UNAVAILABLE` отдельно.** Если у
> vantage'а упал control-endpoint, все его таргетные verdicts становятся
> `VANTAGE_UNAVAILABLE`. С формулой `affected = count(...,"ne","OK")`
> такая точка считается «affected». На корректность триггеров это **не
> влияет**, потому что `VANTAGE_UNAVAILABLE` — LOW-confidence и в список
> `discriminator_any` не входит. Гейт `discriminator_any ≥ 1` гарантирует:
> сломанный vantage в одиночку никогда не поднимет ложный Info/Warning.

Каждый из 4 consensus-триггеров заканчивается финальным AND-условием
`last(/.../dpi.discriminator_any[T,K,P])>=1`, поэтому **alert поднимается
только если хотя бы один vantage увидел чёткую DPI-сигнатуру**. Голый
`PORT_FILTERED` (LOW confidence — двусмысленно: outbound :25 anti-spam у
ISP, фаервол на target'е, generic drop) попадает в Latest data для
forensic-анализа, но никого не паджит. Шкала зашита арифметически через
`dpi.affected` (литеральные числа в `count(#3,"eq",N)=3`, поскольку
Zabbix не принимает `last(...)` в pattern-аргументе `count`), плюс
защитные условия по `dpi.vantages_total` — **шкала автоматически
масштабируется** при добавлении/удалении vantages, включая случай
`{$DPI.EXCLUDE_KINDS}`, когда разные target'ы видны разному числу
vantage'ей:

| Условие (3× подряд) | Severity | Семантика | Защитное условие |
|---------|:--------:|-----------|------|
...
| `affected = 2` AND `discriminator_any ≥ 1` | Warning  | 2 vantage не видят — directional-блок одного региона | `vantages_total ≥ 3` |
| `affected ≥ 3 AND peers_ok ≥ 1` AND `discriminator_any ≥ 1` | Average | большинство (но не все) | `vantages_total ≥ 4` |
| `peers_ok = 0` AND `discriminator_any ≥ 1` | High | глобально недоступен с подтверждённой DPI-сигнатурой | (нет защитного условия) |
...
`{$DPI.INTERVAL} = 5m`. Защитные условия (`vantages_total ≥ N`) держат
тиры взаимоисключающими на маленьких mesh-ах:
...
Sentinel **Info**-триггер `DPI Mesh stale` поднимается, если calc item
...
severity-tier'а зависят (`dependencies`) от этого sentinel-а: мёртвый aggregator
не должен генерировать ложные High-ы.

```mermaid
flowchart TB
    subgraph Vantages["Vantage хосты: probe → /dpi.verdict[T,K,P]"]
      V1[vantage A · EU]
      V2[vantage B · BY]
      V3[vantage C · RU/ISP-1]
      V4[vantage D · RU/ISP-2]
      V5[vantage E · RU/ISP-3]
    end
    subgraph Aggr["dpi-mesh-aggregator (phantom host)"]
      P["last_foreach(/*/dpi.verdict[T,K,P])<br/>→ peers_ok, affected, vantages_total,<br/>discriminator_any"]
      P --> T{severity tier?<br/>+ AND discriminator_any ≥ 1}
      T -->|affected=1| I["ℹ️ Info"]
      T -->|affected=2| W["⚠️ Warning"]
      T -->|affected≥3, peers_ok≥1| A["🟠 Average"]
      T -->|peers_ok=0| H["🔴 High"]
      STALE["📭 Info-sentinel: stale<br/>(нет обновлений calc > 1h)"]
      STALE -.suppresses.-> I
      STALE -.suppresses.-> W
      STALE -.suppresses.-> A
      STALE -.suppresses.-> H
...
**Cross-vantage матрица** — пример того, как Zabbix Latest data рендерит один
и тот же target по 5 vantage'ям (имена точек условные):

| target/kind/port | A · EU | B · BY | C · RU/ISP-1 | D · RU/ISP-2 | E · RU/ISP-3 |
|---|---|---|---|---|---|
...
В этом примере fires (severity подбирается по `peers_ok` / `affected`):
- `blocked-service.com`: affected=2, peers_ok=3, discriminator_any=1 → **Warning** ⚠️
- `mail.example.com`: affected=1, peers_ok=2, discriminator_any=0 → **не fires** (нет
  DPI-сигнатуры — `PORT_FILTERED` без подтверждения — это типично outbound :25
  ISP-фильтр, forensic-only)

На остальных vantage'ях `blocked-service.com` не fires (verdict=OK, debounce =3
не достигнут). Если бы все 5 vantage увидели non-OK на одном таргете с
подтверждённой DPI-сигнатурой — fired бы **High** 🔴 (полностью недоступный
сервис плюс ТСПУ/RKN-сигнатура).
...
4 calc items per tuple (`dpi.peers_ok`, `dpi.vantages_total`, `dpi.affected`,
`dpi.discriminator_any`) и 5 триггеров — четыре severity-tier + один
Info-sentinel «stale». Первый calc-цикл = через `{$DPI.INTERVAL}` = 5m.
...
2. **Re-import vantage template** с `Delete missing → Items / Triggers / Discovery rules: yes`
   ...
   - 4 severity trigger prototypes (Info/Warning/Average/High в старых версиях
     назывались Warning/Average/High/Disaster — оба варианта подлежат удалению)
...
./deploy/deploy-to.sh <vantage-host> --from-git https://github.com/IT-for-Prof/zabbix-dpi-checker.git main
...
```bash
./deploy/deploy-to.sh <vantage-host>   # rsync + ssh + установщик в одну команду
```

### Способ 3. Ручной запуск установщика на хосте

```bash
ssh root@<vantage-host>
